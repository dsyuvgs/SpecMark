# _fix_node3.py — 仅修复节点3：插入 apply_priority_rules 函数
...
# 节点3: iron_classify 的 return None 在 "已读回执" 之后
# 后面紧跟 def is_header_row
old_node3_end = '''                  if re.search(r'已读回执|已读', text):
                      return ('体验', '中性', '低')
                  return None

              def is_header_row'''

new_node3_end = '''                  if re.search(r'已读回执|已读', text):
                      return ('体验', '中性', '低')
                  return None
...
count = content.count(old_node3_end)
if count != 1:
    print(f'ERROR: 匹配到 {count} 处，期望 1 处')
else:
    content = content.replace(old_node3_end, new_node3_end)
    print('节点3 HELPER_FUNC 已插入')
